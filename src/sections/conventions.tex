			...
			\begin{center}
				\textcolor{background}{
					\begin{tabularx}{\textwidth}{>{\columncolor{rowcolor1}}X|>{\columncolor{rowcolor2}}p{5cm}}
						...
						\tablebash{HEAD\^3} & Shorthand for 3 commits before current \tablebash{HEAD}. \\
					\end{tabularx}
				}
			\end{center}
...
\begin{tikzpicture}
	\node [mybox, fill=boxcolor, draw=outlinecolor] (box){%
		\begin{minipage}{0.3\textwidth}
			...
			Reasons keeping \inlinebash{master} as the default in older project include:
			\begin{itemize}
					\item This switch can cause backwards compatibility issues with earlier project releases
					\item Complicated CI/CD processes can consequently fail in non-obvious (difficult to troubleshoot) ways
